...
export default function ProductsUser({ history }) {
  ...
  var [productQte, setproductQte] = useState();
 ...
 const  handlePayLivraison=()=>{
   const mode="home"
  Axios.post(`${process.env.REACT_APP_URL}booking/prod/confirm/pay/${id}/${mode}`, {
    headers: {
  ...
  const handleDelete=(product)=>{
    ...
    Axios.post(`${process.env.REACT_APP_URL}booking/prod/delete/product/${user}/${id}`, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }).then((response) => {
      console.log(response);
      toast.error("product deleted from bascket");
      window.location.reload();
    });
  }
...
const onSubmitQte=(e)=>{
  ...
  Axios.post(`${process.env.REACT_APP_URL}booking/prod/update/${id}/${productId}/${productQte}`, {
    headers: {
      "Content-Type": "multipart/form-data",
    },
  }).then((response) => {
    console.log(response);
    toast.success("product number places updated");
    window.location.reload();
  });
}
  const all = products.map((e) => {
    return (
      <tr>
        <td>{e.name}</td>
        <td>
        ...
        <td>
        <button  className="btn btn-sm btn-danger" style={{marginRight:"1rem"}} onClick={() => handleDelete(e.productId)}>
            <i className="tim-icons icon-trash-simple"></i>
          </button>
          <button  className="btn btn-sm btn-success" style={{marginRight:"1rem"}} onClick={() => handleQte(e.productId,e.name,e.qte,e.img[0])}>
  ...
  return (
    ...
    <ModalBody>
      <img
      alt={productName}
      className="avatar" src={productImg}
      />
      ...
      <Label htmlFor="placesNembre">Number reservation</Label>                 
                  ...
                  value={productQte}
                  onChange={ ( data ) => {
                    setproductQte(data.target.value);
                } }
                />
<FormGroup>
