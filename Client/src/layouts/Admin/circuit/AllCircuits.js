...
export default function PlaceAdmin({ history }) {
  ...
  setLoading(true)
    ...
    Axios.post(`${process.env.REACT_APP_URL}circuit/upload-images`, data, {
    }).then(res => {
...
setLoading(true)
  ...
  Axios.post(`${process.env.REACT_APP_URL}circuit/upload-audio/${id}`, data, {
  }).then(res => {
      console.log(res.data)
      setLoading(false);
...
setLoading(true)
  ...
  Axios.post(`${process.env.REACT_APP_URL}circuit/upload-video/${id}`, data, {
  }).then(res => {
      console.log(res.data)
      setLoading(false);
...
setLoading(true)
  ...
  Axios.post(`${process.env.REACT_APP_URL}circuit/circuit/${id}`,{"name":nameC}, {
  }).then(res => {
      console.log(res.data)
      setLoading(false);
  ...
  const  columns = [
    ...
    },
    {
      ...
      <button style={{marginRight:"1rem"}}  onClick={() => confirmation(d._id)}>
          ...
          <button style={{marginRight:"1rem"}}  onClick={() => handleToggleAudio(d._id)}>
            <i className="tim-icons icon-triangle-right-17"></i>
          </button>
          <button style={{marginRight:"1rem"}}  onClick={() => handleToggleVideo(d._id)}>
  ...
  const handleUpdate = async (id) => {
    ...
    data.append("name", formData.namePlace);
    data.append("desc", formData.descriptionPlace);

    toast.success("Place updated successfully")


    await Axios.patch(`${process.env.REACT_APP_URL}circuit/${id}`, data, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }).then((response) => {
  ...
  const handleDelete = (id) => {
    // console.log("Delete");
    Axios.delete(`${process.env.REACT_APP_URL}circuit/${id}`, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }).then((response) => {
  ...
  const { name, desc,image } = formData;
      ...
      <tr>
        ...
        <td>
          <img
            alt={ev.name}
            className="avatar"
            src={process.env.PUBLIC_URL + `/uploads/${ev.im}`}
    ...
    <>
      ...
      <aside style={thumbsContainer}>{thumbs}</aside>
          ...
          <Modal isOpen={isToggled} toggle={isToggled}>
            <ModalHeader toggle={isToggled}>Add Circuit</ModalHeader>
            <ModalBody>
              <Form onSubmit={onSubmit} method="Post">
                <FormGroup>
                  <Label htmlFor="namePlace">name</Label>
                  <Input
                    color="info"
                    type="text"
                    style={{color: '#000'}}
...
<Card>
              <CardHeader>
                <CardTitle tag="h4">List of Circuit</CardTitle>
              </CardHeader>
              <CardBody>
              <DataTableExtensions
      columns={columns}
