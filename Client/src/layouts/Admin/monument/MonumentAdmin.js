...
export default function MonumentsAdmin({ history }) {
  var [monuments, setMonuments] = useState([]);
  ...
  const onSubmit=(e)=> {
    ...
    Axios.post(`${process.env.REACT_APP_URL}monuments/upload-images`, data, {
    }).then(res => {
  ...
  const loadCastronomy = () => {
    ...
    Axios.get(`${process.env.REACT_APP_URL}monuments/place/${placeId}`, {
      headers: {
...
>
  ...
  //  console.log(formData.nameJeux);
   // console.log("file" + file.);
    ...
    data.append("desc", formData.descriptionJeux);
  ...
  //  console.log("file" + file.nameCastronomy);
    ...
    data.append("name", formData.nameJeux);
    data.append("desc", descript);
    data.append("details", details);
    toast.success("Monument updated successfully")


    await Axios.patch(`${process.env.REACT_APP_URL}monuments/${id}`, data, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }).then((response) => {
  ...
  const handleDelete = (id) => {
    // console.log("Delete");
    Axios.delete(`${process.env.REACT_APP_URL}monuments/${id}`, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }).then((response) => {
      console.log(response);
      toast.error("Monument deleted");
      window.location.reload();
    });
  };
      ...
      <div className="content">
        ...
        <Row>
            ...
            <Modal isOpen={isUpdated} toggle={isUpdated}>
                ...
                <FormGroup>
                  <Label htmlFor="nameJeux">Name</Label>
                  <Input
                    color="info"
                    type="text"
                    style={{color: '#000'}}
          ...
          <Col md="12">
            {
              ...
              (
                <Card>
                <CardHeader>
                  <CardTitle tag="h4">Monument</CardTitle>
                </CardHeader>
                <CardBody>
                <DataTableExtensions
        columns={columns}
