...
export default function AccomodationAdmin({ history }) {
  ...
  const onSubmit=(e)=> {
    ...
    data.append("image", file);
    data.append("name", formData.nameMaison);
    data.append("desc", descript);
    data.append("price", formData.pricesMaison);
    data.append("phone", formData.phoneMaison);
    data.append("adress", formData.adress); 
    for (const key of Object.keys(image1)) {
        data.append('imgCollection', image1[key])
    }
    Axios.post(`${process.env.REACT_APP_URL}accomodation/upload-images`, data, {
    }).then(res => {
  ...
  const loadMaison = () => {
    Axios.get(`${process.env.REACT_APP_URL}accomodation`, {
      headers: {
  ...
  //  console.log(formData.image);
    ...
    data.append("image", file);
    data.append("name", formData.nameMaison);
    data.append("desc", formData.descriptionMaison);
    data.append("price", formData.pricesMaison);
    data.append("phone", formData.phoneMaison);
    data.append("adress", formData.adress);
  ...
  const handleDelete = (id) => {
    // console.log("Delete");
    Axios.delete(`${process.env.REACT_APP_URL}accomodation/${id}`, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }).then((response) => {
    ...
    return (
      <tr>
        ...
        <td>
          ...
          <button className="btn btn-sm btn-primary" style={{marginRight:"1rem"}} onClick={()=>handletoggleUpdate(ev)}><i className="tim-icons icon-pencil"></i></button>
         
          <button className="btn btn-sm btn-danger" style={{marginRight:"1rem"}} onClick={() => handleDelete(ev._id)}>
            <i className="tim-icons icon-trash-simple"></i>
          </button>
        </td>
      </tr>
